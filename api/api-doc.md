...
```json
{
  ...
  "data": [ // 直接返回一个扁平的数组，每个对象都是一个搜索结果
    {
      "item_type": "file", // 明确类型是文件还是文件夹 后端枚举类ItemType
      "id": 201,
      "name": "项目报告.pdf",
      "path": "/文档/工作/项目报告.pdf", // ✨ 后端直接生成好的可读路径
      "file_type": "document",//后端枚举类 FileTypeEnum
      "file_size": 2097152,
      "file_status": "available",//后端枚举类 FileStatusEnum
    ...
    "folders": [
      {
        "id": 101,
        "name": "我的文档",
        "folder_type": "notebook",//枚举 FolderTypeEnum
        "sub_item_count": 5,
...
1. **执行下载**：客户端收到 DownloadSessionResp 后，遍历 items 数组。
    - 如果 type 是 directory，则在本地创建对应目录。
    - 如果 type 是 file，则使用提供的 download_url 下载文件到 relative_path。
2. **更新状态**：在每个文件下载结束时（成功、失败、取消），客户端调用 PUT /transfers/{log_id}/status (见 5.5 节) 来更新该任务的最终状态。
    ...
    "records": [
      {
        ...
        "status": "success",//log枚举 TransferStatusEnum
        "create_time": "2024-07-25T10:00:00Z"
      },
      {
        "log_id": 2,
        ...
        "item_size": 5242880,
        "transfer_type": "upload",//TransferTypeEnum
        "status": "failed",//log枚举 TransferStatusEnum
...
**描述:** 清空所有已完成（成功或失败）的传输记录。**此操作仅删除日志，不影响实际文件（可选）**。
...
**描述:** 在指定的笔记本下创建一个新的、空的 Markdown 文档。
...
| file_id     | Object | **是**   | 当前正在阅读或编辑的文件的ID。 |
